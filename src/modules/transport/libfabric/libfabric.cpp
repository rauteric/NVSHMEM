...
int nvshmemt_libfabric_ack_aggregator::flush_peer(
    ...
    nvshmemt_libfabric_gdr_amo_ack_op_t *ack = NULL;
    uint64_t num_retries = 0;
    int status;
    ...
    ack->ack_header = NVSHMEMT_LIBFABRIC_MSG_COALESCED_ACK;
    ...
    do {
        status = fi_send(ep->endpoint, send_elem,
                         sizeof(nvshmemt_libfabric_gdr_amo_ack_op_t),
...
static int nvshmemt_libfabric_gdr_process_completion(nvshmem_transport_t transport,
    ...
    if (entry->flags & FI_SEND) {
        ...
        if (coal_ack->ack_header == NVSHMEMT_LIBFABRIC_MSG_COALESCED_ACK) {
            nvshmemt_libfabric_signal_state_t *signal_state =
        ...
        if (op->type == NVSHMEMT_LIBFABRIC_AMO_ACK_SEND) {
            nvshmemt_libfabric_gdr_amo_ack_op_t *ack_op =
