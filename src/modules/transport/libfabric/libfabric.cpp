    ...
    if (libfabric_state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
        ...
        if (progress_qp_index != NVSHMEMX_QP_HOST) {
            if (libfabric_state->proxy_signal_state.ack_aggregator &&
                libfabric_state->eps.size() > NVSHMEMT_LIBFABRIC_PROXY_EP_IDX) {
                status = libfabric_state->proxy_signal_state.ack_aggregator->flush_all(
                    transport, libfabric_state->eps[NVSHMEMT_LIBFABRIC_PROXY_EP_IDX]);
                if (status) {
                    libfabric_state->signal_queue_lock.clear(std::memory_order_release);
                    return NVSHMEMX_ERROR_INTERNAL;
                }
...
int nvshmemt_libfabric_ack_aggregator::flush_all(
    ...
    for (int pe : dirty_peers) {
        fi_addr_t dest_addr = pe * libfabric_state->num_selected_domains + ep->domain_index;
...
/* Drain done_queue: Thread A does fi_recv re-post + fi_send response + coalesced ACK */
static int nvshmemt_libfabric_gdr_complete_amos(nvshmem_transport_t transport) {
    nvshmemt_libfabric_state_t *libfabric_state = (nvshmemt_libfabric_state_t *)transport->state;
    signal_delivery_done_entry done;
    uint64_t num_retries = 0;
...
static int nvshmemt_libfabric_finalize(nvshmem_transport_t transport) {
    ...
    if (state->proxy_signal_state.ack_aggregator) {
        if (state->eps.size() > NVSHMEMT_LIBFABRIC_PROXY_EP_IDX) {
            state->proxy_signal_state.ack_aggregator->flush_all(
                transport, state->eps[NVSHMEMT_LIBFABRIC_PROXY_EP_IDX]);
        }
        delete state->proxy_signal_state.ack_aggregator;
    }
    if (state->host_signal_state.put_signal_seq_counter_per_pe)
